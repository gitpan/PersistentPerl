...
use vars qw(
    $_shutdown_handler $i_am_perperl %_opts $_opts_changed @_cleanup $_sub
    @_shutdown
);
...
# Return true if we are running under persistentperl, false otherwise.
sub i_am_perperl { defined($i_am_perperl); }

# Set one of the persistentperl options.
...
# Return one of the persistentperl options.
...
## bootstrap PersistentPerl $VERSION;
...
"C<-w>" and will pass the "C<-t>" option to PersistentPerl, setting the
...
option name in upper-case.  For example to set the perperl Timeout option, use
the environment variable named PERPERL_TIMEOUT.
...
be Persistent followed by the option name.  For example to set the
...
Returns a boolean telling whether this script is running under PersistentPerl
or not.  A perl script can run under regular perl, or under PersistentPerl.
...
test to make sure both the PersistentPerl module is available and you are running
under PersistentPerl:
...
Set one of the PersistentPerl options given in L<"Options Available">.  Returns
...
for your OS, or compile PersistentPerl from source code.  See L<"DOWNLOADING">
...
with.  PersistentPerl is known to work under Solaris, Redhat Linux,
FreeBSD and OpenBSD.  There may be problems with other OSes or
earlier versions of Perl.  PersistentPerl may not work with threaded perl -- as
...
the mod_persistentperl module if you have the command B<apxs> in your path.
...
you run "perl Makefile.PL".  The PersistentPerl binaries and module will then
...
Compile as above, then manually copy the perperl and perperl_backend binaries to
...
To compile the optional apache mod_persistentperl module you must have the B<apxs>
...
Copy the mod_persistentperl.so from the mod_persistentperl directory, or from the
mod_persistentperl2/.libs directory, to wherever your apache modules are stored
...
The only benefit of this over normal perl is that perperl will pre-compile
your script.
...
Group names are created by the person running PersistentPerl based on
their needs.  There are two special group names "none" and "default".
All other group names are created by the user of PersistentPerl using the
...
=head2 PersistentPerl Home Page
...
Archives and subscription information are at http://lists.sourceforge.net/lists/listinfo/persistentperl-users
...
Archives and subscription information are at http://lists.sourceforge.net/lists/listinfo/persistentperl-announce
